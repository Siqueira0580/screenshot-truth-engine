...
export default function TermsInterceptor({ children }: TermsInterceptorProps) {
  ...
  // BLINDAGEM: enquanto loading, não avaliar nada
  if (loading) {
    return (
  ...
  // Sem user = não bloquear (ProtectedRoute cuida disso)
  if (!user) return <>{children}</>;

  // Perfil carregado e termos já aceites
  if (profile?.terms_accepted !== false) return <>{children}</>;

  // Termos NÃO aceites — mostrar overlay
  return (
